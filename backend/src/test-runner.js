...
module.exports = async function runTest() {
  ...
  try {
    // Clean up previous test users and profiles
    console.log('Cleaning up previous test databases records...');
    await StudentProfile.deleteMany({
      $or: [
        { email: { $regex: /test_student/i } },
        { email: 'student@pmss.gov.in' },
        { aadhaar: '123456789012' }
      ]
    });
    
    const usersToDelete = await User.find({
      email: { $in: [/test_student/i, 'student@pmss.gov.in'] }
    });
    ...
    formData.append('casteCertificate', new Blob([dummyPdfBuffer], { type: 'application/pdf' }), 'dummy.pdf');
    ...
    const profilePayload = {
      ...
      documents: {
        ...
        casteCertificate: uploadData.data.casteCertificate
      }
    };
    ...
    const applicationPayload = {
      ...
      documents: {
        ...
        casteCertificate: profileUpdateData.data.documents.casteCertificate,
